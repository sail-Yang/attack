...
class TargetAttackGAN(nn.Module):
  ...
  def build_model(self):
    ...
    # self.generator = GeneratorResnet().cuda()
  ...
  def train(self, train_loader, target_labels, train_labels, database_loader, database_labels, num_database, num_train, num_test):
    ...
    smooth_l1_loss = torch.nn.SmoothL1Loss(reduction='mean')
    # optimizers
    ...
    for epoch in range(self.args.epoch_count, total_epochs):
      ...
      for i, data in enumerate(train_loader):
        ...
        if i % self.args.sample_checkpoint == 0:
          ...
          adv_0_img = torch.where(x_0<0.5, torch.zeros_like(x_0), torch.ones_like(x_0)).clone().detach()
          logger.info("l0: {:.7f}".format(torch.norm(adv_0_img, 0)/args.batch_size))
        ...
        if i % self.args.checkpoint == 0:
          ...
          # )
          logger.info(
            'step: {:3d} g_loss: {:.3f} d_loss: {:.3f} hash_loss: {:.3f} l1_loss: {:.7f} qua_loss: {:.7f} smooth_loss: {:.7f}'
                        .format(i, fake_g_loss, d_loss, logloss, l1_loss, qua_loss, smooth_loss)
          )
    self.save_generator()
  
  def test(self, target_labels, database_loader, test_loader, database_labels, test_labels, num_database, num_test):
    self.prototype_net.eval()
    test_generator = Generator(evaluate=True)
    # test_generator = GeneratorResnet(evaluate=True)
    gen_path = os.path.join(self.args.save_path, self.args.attack_method, 'generator_test_{}.pt'.format(self.model_name))
    test_generator.load_state_dict(torch.load(gen_path))
    test_generator.eval()
    ...
    for it, data in enumerate(test_loader):
      ...
      target_fake, _, _, _ = self.generator(data_input, feature)
